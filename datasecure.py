...
def decrypt_text(encrypt_text, key):
    try:
        cipher= Fernet(generate_key(key))
        return cipher.encrypt(encrypt_text.encode()).decode()
    except:
        return None
...
if choice == "Home":
    ...
    st.markdown("""
    Develop a Streamlit-based secure data storage and retrieval system where:Users store data with a
    unique passkey.Users decrypt Users decrypt data by providing the correct passkey.Multiplefailed attempts 
    ...
    if st.button("Register"):
        ...
        if st.button("Login"):
            if username in stored_data and stored_data[username]["password"] == hash_password(password):
                ...
                st.error(f"❌ Invalid Credentials! Attempts left: ")
